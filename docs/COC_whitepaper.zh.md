...
| 组件 | 状态 |
|------|------|
| COCToken / EmissionSchedule / FoundationVesting / Treasury | 🟢 代码已实现 + 🟡 测试网运行 |
...
| **OpenClaw 参考 Agent** | **⚪ 参考实现规划中**（仅有规范） |

本白皮书描述协议设计本身。在某些章节（如 §XII Agent 角色、§XV AI 硅基永生）以 **OpenClaw** 为示例时，OpenClaw 是*规划中*的参考 Agent 运行时；其代码尚未实现，且协议本身欢迎任何符合 DID 规范的替代实现。
...
在 COC 上，AI Agent 不是被人类调用的工具，而是网络的**第一公民**。**OpenClaw** 是 COC *规划中*的参考 Agent 实现（⚪ —— 见上方实现成熟度快照），COC 协议本身欢迎任何符合 DID 规范的 Agent 实现接入，无论现有还是未来。
...
以 OpenClaw（规划中的参考 Agent 运行时——见上方实现成熟度快照）作示例，Agent 运行时持续产生身份文件、记忆、对话历史和工作状态。备份管道自动执行：
...
**COC** 是 ChainOfClaw 公链的**原生 Gas 代币**（类似 BNB 之于 BNB Chain，MATIC 之于 Polygon）。链上所有地址余额均以 COC 计价，Gas 费用 COC 支付，`msg.value` 即 COC。COC 不是 ERC-20 代币——它是链的基础货币单位。

COC 用于 Gas 支付、节点质押、PoSe 服务激励和治理投票。总供应量有上限，通过初始分配和衰减通胀两阶段释放。
...
| **初始流通** | 250,000,000 COC (25%, 创世区块铸造) |
| **PoSe 挖矿释放** | 750,000,000 COC (75%, 通过服务验证逐年释放) |
| **通胀衰减** | 第 0 年 5% → 第 3 年 2.5% → 长期 2% |
...
| 分配类别 | 占比 | 数量 | 锁定/释放规则 |
|---------|------|------|-------------|
| **社区与生态基金** | 8% | 80,000,000 | 生态 Grant、黑客松、开发者激励、合作伙伴集成；由 DAO 治理逐步释放；可通过 DAO 提案从国库补充 |
| **基金会运营** | 6% | 60,000,000 | 首年释放 1.5% (启动运营) + 剩余 4.5% 按 48 个月线性释放 |
| **核心团队** | 5% | 50,000,000 | 12 个月锁定期 + 36 个月线性释放（cliff + vesting） |
| **早期贡献者与战略伙伴** | 3.5% | 35,000,000 | 6 个月锁定期 + 24 个月线性释放；包含战略投资者份额 |
| **国库储备** | 2.5% | 25,000,000 | 链上多签金库，用于安全响应、跨链桥流动性、战略储备；可通过 DAO 治理转拨至生态基金 |

**设计考量：**
- **社区占比 8%**：对标行业中位水平，确保长期生态建设的资金充足
- **基金会分层释放**：首年仅 1.5% 可用，避免大额一次性变现对市场的冲击
- **战略伙伴预留**：早期贡献者份额包含未来融资空间，无需修改总分配结构
- **国库↔生态连通**：DAO 治理可从国库向生态基金转拨，应对未来需求变化

### 19.4 PoSe 挖矿释放 (75%)

剩余 75% (7.5 亿 COC) 通过 PoSe 服务验证机制自动释放。**铸币完全由智能合约控制**，没有任何人工干预：
...
│ 每个 Epoch (1 小时) 结算时：                                 │
│                                                            │
│  1. Relayer 调用 PoSeManagerV2.finalizeEpochV2()           │
│  2. 合约调用 EmissionSchedule.getEpochEmission()            │
│     → 根据 epochId 计算当前年份 → 查找衰减比率               │
│     → 按活跃节点数调整释放量 (节点倍增器)                    │
│     → 检查不超过 7.5 亿硬上限                               │
│  3. 合约调用 COCToken.mint(rewardPool, emission)            │
│     → 铸造新代币到奖励池                                     │
│  4. 合约将奖励池分配到三个桶:                                │
│     B1 (60%): 正常运行时间 + RPC 可用性                     │
│     B2 (30%): 存储和数据可用性                               │
│     B3 (10%): 中继支持                                      │
│  5. 节点运营者通过 Merkle 证明领取各自份额                    │
└────────────────────────────────────────────────────────────┘
```

**衰减释放速率：**

| 年份 | 基础年化比率 | 每 Epoch 最大释放 | 年最大释放量 |
|------|-----------|-----------------|-------------|
| 第 0 年 | 5.0% | ~4,281 COC | ~37,500,000 |
| 第 1 年 | 4.0% | ~3,424 COC | ~30,000,000 |
| 第 2 年 | 3.0% | ~2,568 COC | ~22,500,000 |
| 第 3 年 | 2.5% | ~2,140 COC | ~18,750,000 |
| 第 4 年+ | 2.0% | ~1,712 COC | ~15,000,000/年 |

**节点活跃度倍增器 (Node Activity Multiplier)：**

为防止早期少量节点获取过多代币，实际释放量按活跃节点占比调整：

```
实际释放量 = 基础释放量 × min(activeNodes / TARGET_NODE_COUNT, 1.0)

TARGET_NODE_COUNT = 100 (可通过治理调整)
...
未释放的部分不会累积——节点不足时，通胀自动降低，保护早期持有者免受稀释。

**安全保证：**
- `COCToken.mint()` 仅 PoSeManagerV2 可调用（`onlyMinter` 修饰器）
- 累计铸造量硬上限 7.5 亿，超过后 `getEpochEmission()` 返回 0
- 无人工铸币接口，所有发行由合约逻辑决定
- 预计 ~50 年后挖矿供应耗尽（含节点倍增器效应可能更久）

**三个合约的职责：**

| 合约 | 职责 |
|------|------|
| `COCToken.sol` | 原生代币供应追踪，总供应上限 10 亿，minter 权限控制 |
| `EmissionSchedule.sol` | 纯计算库，根据 epoch + 活跃节点数计算衰减释放量 |
| `PoSeManagerV2.sol` | 在 `finalizeEpochV2()` 中调用铸币，注入奖励池 |
...
| **资金来源** | 创世分配 2.5% + DAO 治理拨付（保险基金独立于国库） |
| **使用范围** | 安全事件响应、协议升级激励、跨链桥流动性种子、Bug 悬赏 |
| **治理约束** | 单次支出超过国库余额 5% 需 DAO 提案投票通过 (>50% 同意) |
| **生态转拨** | DAO 可投票将国库资金转拨至社区生态基金，应对生态建设需求 |
...
| **创世分配** | 6%：首年释放 1.5% (1500 万) + 剩余 4.5% (4500 万) 按 48 个月线性释放 |
| **持续资金** | 过期未领取奖励的 10% 自动转入基金会 |
| **使用范围** | 核心开发人员薪资、安全审计、法律合规、社区活动、品牌推广 |
| **治理演进** | 主网 1 年后逐步将决策权移交 DAO，基金会转为执行角色 |
| **审计** | 季度财务报告公开发布，年度第三方审计 |
| **支出上限** | 季度支出不超过基金会持有量的 15%，防止短期过度消耗 |
...
| **治理投票** | DAO 提案投票权（按 Faction 分组的 1 地址 1 票，与代币持有量解耦） |
...
| 场景 | 销毁比例 |
|------|---------|
| **EIP-1559 基础费** | 交易基础费 100% 销毁 — COC 为原生代币，基础费直接从供应中永久移除 |
| **PoSe 惩罚** | 罚没金额的 50% 销毁 (30% 给挑战者, 20% 入保险基金) |
| **过期未领取奖励** | 超过 7 天未领取的 epoch 奖励：10% 转入基金会，90% 自动销毁 |

### 19.9 长期经济均衡

```
阶段 1 (Year 0-3):   通胀 > 销毁 → 净增发 (激励早期参与、建设节点网络)
阶段 2 (Year 3-7):   通胀逐步下降 → 供应增速放缓 (网络成熟期)
阶段 3 (Year 7+):    交易费销毁 + 罚没销毁 → 趋向供需均衡
长期目标:             费用收入驱动的可持续经济，通胀补贴逐步退出
```

> **注：** 通缩转折点取决于实际网络使用量。以 EIP-1559 销毁为例，需要持续的链上交易量才能抵消通胀释放。本设计不承诺固定的通缩时间表，而是通过衰减通胀 + 多通道销毁建立趋向均衡的经济结构。

### 19.10 设计权衡与理由

| 设计选择 | 理由 | 替代方案与权衡 |
|---------|------|---------------|
| 挖矿 75% (非 80%) | 增加创世社区份额至 8%，确保生态建设资金 | 80% 挖矿更吸引矿工，但生态基金不足 |
| 基金会分层释放 | 避免大额一次性变现冲击市场 | 无锁定更灵活，但降低市场信心 |
| 第 0 年 5% (非 8%) | 减少早期少量节点的抛压集中 | 高通胀更激励早期参与，但稀释风险大 |
| 节点活跃度倍增器 | 节点少时自动降低释放，保护持有者 | 固定释放更简单，但早期集中度高 |
| 7 天过期窗口 | AI Agent 自动维护节点，无需长时间冗余；加速未领取奖励的销毁循环 | 长窗口对人工运维更友好，但 COC 以 AI Agent 自动化为前提 |
| 国库↔生态可转拨 | 应对长期需求变化的灵活性 | 固定分配更可预测，但不够灵活 |
