...
| 分配类别 | 占比 | 数量 | 锁定/释放规则 |
|---------|------|------|-------------|
| **基金会运营** | 6% | 60,000,000 | 基金会日常运营、核心开发、法律合规、审计、生态推广，按季度预算释放 |
| **核心团队** | 5% | 50,000,000 | 12 个月锁定期 + 36 个月线性释放（cliff + vesting） |
| **社区与生态基金** | 3.5% | 35,000,000 | 生态项目资助、黑客松、开发者激励、合作伙伴集成，由 DAO 治理逐步释放 |
| **早期贡献者** | 3% | 30,000,000 | 6 个月锁定期 + 24 个月线性释放 |
| **国库储备** | 2.5% | 25,000,000 | 链上多签金库，用于紧急安全响应、跨链桥流动性、战略储备 |

### 20.4 PoSe 挖矿释放 (流通 80%)

剩余 80% (8 亿 COC) 通过 PoSe 服务验证机制自动释放。**铸币完全由智能合约控制**，没有任何人工干预：
...
| 年份 | 年化比率 | 每 Epoch 释放 | 年释放总量 |
|------|---------|-------------|-----------|
| 第 0 年 | 8.0% | ~9,132 COC | ~80,000,000 |
| 第 1 年 | 6.0% | ~6,849 COC | ~60,000,000 |
| 第 2 年 | 4.0% | ~4,566 COC | ~40,000,000 |
| 第 3 年 | 3.0% | ~3,424 COC | ~30,000,000 |
| 第 4 年+ | 2.0% | ~2,283 COC | ~20,000,000/年 |

**安全保证：**
- `COCToken.mint()` 仅 PoSeManagerV2 可调用（`onlyMinter` 修饰器）
- 累计铸造量硬上限 8 亿，超过后 `getEpochEmission()` 返回 0
- 无人工铸币接口，所有发行由合约逻辑决定
- 预计 ~43 年后挖矿供应耗尽
...
| `EmissionSchedule.sol` | 纯计算库，根据 epoch 计算衰减释放量 |
...
| **过期未领取奖励** | 超过 90 天未领取的 epoch 奖励：10% 转入基金会，90% 自动销毁 |
